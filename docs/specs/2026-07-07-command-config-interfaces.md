...
| `allow-rename` | on/off | off (**AMENDED, SP7 Task 3 — closes follow-up #52**: default corrected from an unverified `on` to tmux's real default, off since 2.6; now LIVE via `Options::allow_rename`, gating ONLY `ESC k`-sourced automatic-rename — OSC 0/2 stays unconditional, see `grid::Grid::title_from_esc_k` in `2026-07-07-parity-polish-interfaces.md`'s `## grid-v2` amendment) |
